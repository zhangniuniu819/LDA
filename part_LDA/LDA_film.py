...
# 配置开关（激进模式）
# ------------------------------
USE_WHITELIST_ONLY = True      # 只保留下面定义的女性主义核心词（推荐）
NOUNS_ONLY = True               # 仅保留名词（如果USE_WHITELIST_ONLY=True，此项自动忽略）
USE_BIGRAM = False              # 关闭bigram，保持简单
# ------------------------------
MOVIES_FILE = 'douban_film_comments_with_sentiment.csv'
TEXT_COL = '评论内容'
NUM_TOPICS = 6                 # 主题数降到4，更聚焦
PASSES = 60
ITERATIONS = 500
NO_BELOW = 8                   # 词至少在12个评论中出现
NO_ABOVE = 0.3                  # 词出现在超过30%评论则过滤

OUTPUT_TXT = 'film_lda_feminist_output.txt'
OUTPUT_HTML = 'film_lda_feminist_vis.html'
MODEL_PREFIX = 'film_feminist'
...
# ------------------------------
# 女性主义核心词白名单（USE_WHITELIST_ONLY=True时只保留这些词）
# ------------------------------
...
# ------------------------------
# 超大停用词表（包含你所有反馈的泛化词，但不包含上面白名单中的词）
# ------------------------------
extra_stop_words = [
    # 从你的主题词中提取的无关词汇（2025-05-26反馈）
    ...
    '吧', '哦', '嗯', '呢', '吗', '呀', '啦', '本书', '作者', '读者', '文字', '阅读',
    '看过', '看完', '一部', '故事', '小说', '作品', '书中', '感觉', '真的', '特别',
    '完全', '其实', '算是', '部分', '整体', '内容', '情节', '人物', '描写', '看到',
    '感受', '理解', '方式', '我们', '他们', '她们', '但是', '最后', '真是', '这部',
    '不错', '不要', '问题', '后面', '感动', '需要', '片子', '纪录片', '叙事', '女主',
    '代孕', '生活', '前面', '还有', '认为', '以为', '感到', '时候', '时间', '地方',
    '事情', '东西', '方面', '情况', '状态', '结果', '第一', '第二', '第三', '首先',
    '其次', '总之', '总的来说', '而且', '然而', '因此', '所以', '不过', '其实', '当然',
    '这样', '那样', '这么', '那么', '这里', '那里', '哪里', '等等', '之类', '能够',
    '应该', '必须', '需要', '也许', '大概', '大约', '几乎', '差不多', '基本上', '大部分',
    '主要', '重要', '现在', '过去', '未来', '今天', '明天', '昨天', '今年', '去年',
    '中国', '国家', '世界', '全球', '国际', '国内', '国外', '地区', '城市',
    '好看', '情感', '文化', '两个', '一定', '成为', '有些', '一起', '观众', '台词',
]
...
def preprocess_text(text):
    ...
    text = re.sub(r'[^\u4e00-\u9fa5]', '', text)  # 只留汉字
    return text

def tokenize_film(text):
...
# LDA 建模
# ------------------------------
def run_lda_film(df, text_col, num_topics, passes):
    print("开始训练电影评论 LDA（女性主义聚焦版）...", flush=True)
    outfile.write(f"\n{'='*60}\n电影评论 LDA 主题分析（白名单模式）\n{'='*60}\n")
    outfile.write(f"白名单仅保留 {len(feminist_core_words)} 个核心词\n\n")

    texts = df[text_col].apply(tokenize_film).tolist()
    texts = [t for t in texts if len(t) >= 2]   # 至少2个核心词
    outfile.write(f"有效文档数（含至少2个核心词）: {len(texts)}\n")
    print(f"有效文档数: {len(texts)}")

    if len(texts) < 50:
        outfile.write("警告：有效文档数过少，请检查数据或放宽过滤条件。\n")
        print("警告：有效文档数过少，可能无足够女性主义讨论。")
...
def main():
    try:
        print("="*60)
        print("豆瓣电影评论 - 女性主义聚焦 LDA")
        print(f"白名单模式: {USE_WHITELIST_ONLY}, 主题数: {NUM_TOPICS}")
        df = pd.read_csv(MOVIES_FILE, encoding='utf-8')
        outfile.write(f"原始评论数: {len(df)}\n")
        print(f"原始评论数: {len(df)}")
        lda = run_lda_film(df, TEXT_COL, NUM_TOPICS, PASSES)
        print("\n🎉 分析完成！")
        print(f"📄 结果: {OUTPUT_TXT}")
        print(f"📊 可视化: {OUTPUT_HTML}")
        print("💡 如果仍有无关词，请手动补充至 feminist_core_words 或 stop_words")
    except Exception as e:
